...
export const Breather = (u: number, v: number, center: vec3 = [0, 0, 0]) => {
  ...
  let x =
    -u +
    (2 * (1 - a * a) * Math.cosh(a * u) * Math.sinh(a * u)) /
      (a *
        ((1 - a * a) * Math.pow(Math.cosh(a * u), 2) +
          a * a * Math.pow(Math.sin(Math.sqrt(1 - a * a) * v), 2)));
  let y =
    (2 *
      Math.sqrt(1 - a * a) *
      Math.cosh(a * u) *
      (-(
        Math.sqrt(1 - a * a) *
        Math.cos(v) *
        Math.cos(Math.sqrt(1 - a * a) * v)
      ) -
        Math.sin(v) * Math.sin(Math.sqrt(1 - a * a) * v))) /
    (a *
      ((1 - a * a) * Math.pow(Math.cosh(a * u), 2) +
        a * a * Math.pow(Math.sin(Math.sqrt(1 - a * a) * v), 2)));
  let z =
    (2 *
      ...
      (-(
        ...
        Math.cos(v) * Math.sin(Math.sqrt(1 - a * a) * v))) /
    (a *
      ((1 - a * a) * Math.pow(Math.cosh(a * u), 2) +
        a * a * Math.pow(Math.sin(Math.sqrt(1 - a * a) * v), 2)));
...
export const SievertEnneper = (
  ...
  const a = 1.0;
  let x =
    Math.log(Math.tan(v / 2)) / Math.sqrt(a) +
    (2 * (1 + a) * Math.cos(v)) /
      (1 + a - a * Math.sin(v) * Math.sin(v) * Math.cos(u) * Math.cos(u)) /
      Math.sqrt(a);
  let y =
    ((2 *
      Math.sin(v) *
      Math.sqrt((1 + 1 / a) * (1 + a * Math.sin(u) * Math.sin(u)))) /
      (1 + a - a * Math.sin(v) * Math.sin(v) * Math.cos(u) * Math.cos(u))) *
...
export const Seashell = (u: number, v: number, center: vec3 = [0, 0, 0]) => {
  let x =
    2 *
    (-1 + Math.exp(u / (6 * Math.PI))) *
    Math.sin(u) *
    Math.pow(Math.cos(v / 2), 2);
  let y =
    1 -
    Math.exp(u / (3 * Math.PI)) -
    Math.sin(v) +
    Math.exp(u / (6 * Math.PI)) * Math.sin(v);
  let z =
    2 *
    (1 - Math.exp(u / (6 * Math.PI))) *
    Math.cos(u) *
